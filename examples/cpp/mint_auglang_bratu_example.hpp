        ...
        namespace mint // min time optimal control
        {
                                     ...
                                     const torch::Tensor &x1)
          {
                                       ...
                                       const TensorDual &params)
            {
              ...
              auto x2 = y.index({Slice(), Slice(2, 3)});
              auto p1 = y.index({Slice(), Slice(0, 1)});
              auto p2 = y.index({Slice(), Slice(1, 2)});
              auto u1star = calc_control(p1,p2,x1,x2);
              auto dp1dt = -a*p1;
              auto dx1dt = x2;

              auto real_dyns = TensorDual::cat({dp1dt, dx1dt});
                                               ...
                                               const torch::Tensor &params)
            {
              ...
              int M = x.size(0);
              int N = 2; // Length of the state space vector in order [p1, x1]
              int D = 3; // Length of the dual vector in order [p1, x1, tf]
              double rtol = 1e-3;
              double atol = 1e-6;
              if (params.dim() == 1)
              {
